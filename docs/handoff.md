...
### 컬러 경계 스냅의 속도 — 측정도 안 된 상태

`snap_mask_to_color_edge` 도입 후 라이브 뷰어가 **7.7 → 1.2 fps** 로 떨어졌다.
파이프라인 택트 예산(1200 ms) 안에 드는지 **아직 안 쟀다.**

원인 짐작 (검증 필요):
- 스냅이 **후보 클러스터마다** 호출된다. 롤러 장면은 후보가 7~18개
- `imageops.gaussian_blur` 가 `np.apply_along_axis` 라 프레임당 여러 번이면 비싸다

고칠 방향: 그래디언트 이미지를 **프레임당 한 번** 계산해 후보들이 공유하게 한다.
`snap_mask_to_color_edge(mask, color, ...)` 시그니처를 미리 계산한 gradient 를
받도록 바꾸면 된다.
...
- `main` = `796ada1` — GitHub https://github.com/Shinhyoung/vision_robotworld 에 푸시됨
- 이번 세션: `2490414` ROI 실측 재설정 → `bbcaadc` 복원 도구 ROI 옵션 →
  `8dafb16` 카메라 재정렬 후 재측정 → `0487590` 롤러 지지면 →
  `e382959` 뷰 합의 → `796ada1` 컬러 스냅
...
| 검출 | — | 10/10 |
| 치수 오차 | — | 최악 축 2.7 % |
| 택트 | 중앙값 258 ms (예산 1200 ms) | **컬러 스냅 후 미측정 — §3** |
...
1. **컬러 스냅 속도** (§3) — 뷰어 7.7 → 1.2 fps, 택트 영향 미측정
2. ⚠️ **부품 미검출 시 `STATUS_NG` → `STATUS_NO_POSE` 수정** (§10)
3. 재구성 메시 폭 +14 % — 분할은 폭을 정확히 재는데(48.6 vs 정답 49) 메시는
...
6. EfficientAD 실학습 (INS-3) — 실촬영 양품으로 학습해야 한다.
   지금 뷰어의 검사 백엔드는 **모의 렌더로 학습**해 실장면 판정이 무의미하다
7. Isaac ROS FoundationPose 브릿지 (POSE-4)
