...
현재 이 저장소는 **ROS 미설치 환경에서 182개 유닛 테스트와 E2E dry-run이 모두 통과**한다.
...
1. **분할**: RANSAC으로 컨베이어 평면 추정 → 평면 위 높이 밴드 → 연결 성분
   - **후보 선별은 2단 게이트다.** 먼저 **스테이션 ROI**(카메라 좌표계 상자)로
     "정지 위치에 있는가"를, 다음으로 **치수 일치**로 "맞는 부품인가"를 본다.
     치수만으로는 벨트 위 다음 부품도 똑같이 통과하므로 위치 게이트가 따로 필요하다
     — 간격 200 mm에서 후속 부품이 선택되던 것을 실측으로 확인했다
   - 두 게이트는 `roboworld_core/segmentation.py` 한 곳에 있고 검사·포즈가 공유한다.
     둘이 서로 다른 물체를 고르면 한 부품을 검사하고 다른 부품을 집게 된다
