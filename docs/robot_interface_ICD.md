...
| 다중 부품 | 프레임 내 최대 1개. 2개 이상이면 가장 큰 클러스터만 처리 |
| 가림(occlusion) | 미지원. 부품이 가려지면 `STATUS_NO_POSE` 또는 낮은 fitness |
...
- [ ] §6.1: grasp의 대칭 불변성 확인 → 불변이 아니면 대응 방식 결정
