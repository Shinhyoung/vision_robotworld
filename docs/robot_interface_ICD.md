...
| 상황 | 발행 결과 |
|---|---|
| 부품이 있고 치수가 맞음 | 정상 처리 (`STATUS_OK` / `STATUS_NG`) |
| 부품 아닌 물체만 있음 (손, 공구, 다른 부품) | `STATUS_NO_POSE` |
| 평면 위에 아무것도 없음 | `STATUS_NO_POSE` |
...
| 상황 | 발행 결과 |
|---|---|
| 상자 안에 치수가 맞는 부품이 있음 | 정상 처리 (`STATUS_OK` / `STATUS_NG`) |
| 부품이 있으나 전부 상자 밖 | `STATUS_NO_POSE` (= 아직 스테이션에 도착 안 함) |
